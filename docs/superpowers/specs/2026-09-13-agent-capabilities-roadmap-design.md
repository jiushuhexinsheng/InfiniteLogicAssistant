...
**关键决策（已在 A 的 spec 阶段定，2026-09-13）**
1. **分层方式**：按风险层级设默认（read/write/exec 各一个动作）+ 单工具/通配规则覆盖；求值顺序 `deny 短路 > rules > tiers > default_action`。默认值**刻意等价于现状**（read 免询问、write/exec 询问），保证升级行为不变。
2. **不做 `allow-always`**：弹窗只有「允许本次 / 拒绝」；想永久免询问就到设置页把该工具配成 `allow`（这本就是持久化路径）。
3. **并发语义**：分流改为按「是否需要确认」——`allow` 的并发（含 write/exec），需确认的逐个串行。**已接受的风险**：两个被 allow 的写操作可能交错。
4. 策略求值失败（配置损坏）时的默认动作 —— 必须 fail-closed。

> **更正（2026-09-13）**：本节原第 1 条曾写「必须修 `TOOLS.risk()` 的 fail-open」，
> 经核实**该断言不成立**，已移除。理由：
> - 未注册的工具名**根本不会执行** —— `TOOLS.acall` 对未知名字直接返回 `Error: unknown tool`；
> - 动态注册的 MCP 工具是显式 `risk="exec"`（`core/tools/mcp_bridge.py:61`），并非默认 read。
>
> 因此 `risk()` 对未知名返回 `"read"` 只是显示/并发分流的怪癖，**不构成可执行路径上的漏洞**。
> 策略层仍会对未知工具名按最严格处理，但那是**纵深防御**，不是补漏。
...
1. ~~**`TOOLS.risk()` 的 fail-open 是 A 的前置缺陷**~~ —— **该断言经核实不成立，已撤回**（详见 A 节末尾的更正）。未注册的工具名不会执行，MCP 动态工具是显式 `exec`。策略层对未知名按最严格处理属纵深防御。
