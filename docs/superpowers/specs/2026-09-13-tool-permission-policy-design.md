...
> ⚠️ **后续变更（2026-09-13 当晚，决策 2 已被取代）**：按用户要求把默认值从「read 免询问、
> write/exec 询问」改为**三档全放行、兜底也放行** —— 决策 2 的「等价于现状」不再成立，那条
> 「升级后行为不变」的迁移属性**已主动放弃**。同时任务级计划确认也改为共用同一份 `permissions`
> （此前它只看 `task.risk`、不受策略影响），放行时仍播报计划但不阻塞。
> 连带影响：`dangerous: true` 的技能（走 `confirm_if_needed`）默认也不再询问。
> 决策 3「不做 allow-always」仍成立 —— 只是现在的默认值本身就等同于 allow-always。
> 收紧手段不变：`permissions.rules`（按工具名 glob，deny 单调短路）或改 `tiers`。
> 决策 4 的并发风险面因此**扩大**：默认全为 `allow`，故所有写操作默认并发执行。
