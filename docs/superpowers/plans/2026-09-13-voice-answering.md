...
上述各项此前标注「未验证」的直接原因是**无法对着麦克风说话**。现补上
`web/scripts/verify-voice.mjs`：它搭场景、采证据、判客观项，人只负责出声与听声。
用法与判据见 README「语音验收台」一节。

其中**第 4 项不需要人说话**（只需助手出声、麦克风听见），曾由验收台自动跑过一次 ——
但**那次「验证」已作废**，理由见下：

| # | 项 | 状态 |
|---|---|---|
| 4 | 助手播报含「小逻小逻」的文本 → 不自触发唤醒 | **⚠️ 对当前实现未经有效验证（原记录作废，需重做）** —— 下表那组证据测的是**已被删除的旧实现**，见下方「撤回说明」 |

> **撤回说明（2026-09-13，唤醒链路重构 Task 8）**：本项原先记为「✅ 已由验收台验证」，
> **该结论对当前实现已无效**，原因如下（分两层，不要把两层混为一谈）：
>
> **一、记录本身在当时是真观测，不是编造。** 这组样本产生于 `ebbd53a`（2026-09-13 18:26），
> 当时 `web/public/lib/wake-word.js` **仍然存在**并定义了 `window.WakeWordEngine`，所以探针读的
> 是一个**活着的引擎**，FAIL 分支当时可达。样本里「播报前 12/22」这些**非零**计数正好印证这一点：
> 若当时探的是一个不存在的全局，前后都会是 0。
>
> **二、但它测的已经不在了。** 随后的唤醒链路重构把 Vosk 引擎、模型、探针目标（`WakeWordEngine`）
> 连同被测的那套门控机制（停引擎 ↔ 释放麦克风）**整体删除并替换**，所以这条「已验证」无法
> 迁移到当前实现 —— 被验证的性质在当前代码里已不存在于那种形态。
>
> 补充一句**需要记住的教训**：Task 7 删掉引擎之后、Task 8 重写检查 4 之前，**同一份检查变成了
> 空洞的** —— 探已删全局取值恒为假、FAIL 分支不可达，「播报期 0/13」那时才成了必然结果。
> 这正是检查 4 必须重写的原因。新检查 4 改为「播报期麦克风**活轨道数归零**（新增
> getUserMedia/track.stop 探针，直接测轨道）**+** 播报窗口内无任何音频上传；取不到证据一律报
> INCONCLUSIVE 而非 PASS」。
>
> **当前状态：第 4 项与其余各项一样，待真人有声环境跑 `npm run verify:voice` 重做。**
> 见 `docs/architecture/roadmap.md` 的 P6 行与 P8，以及 README「语音验收台」。
> 下表样本**仅作历史留痕**，不得再作为任何「已验证」的依据。

```
播报文本：小逻小逻
引擎运行采样数 —— 播报前 12/22，播报期 0/13，播报后 1/1
（播报前后都在跑、只有播报期不跑 = 门控确实生效；若播报期也在跑则判 FAIL）
进入 recording 的采样数：0
状态序列：listening → thinking → listening → done
```

~~读法：播报**前**引擎在 22 次采样里有 12 次在运行、播报**后** 1/1 在运行，只有播报**期**
13 次采样**全部**停止 —— 这排除了「引擎压根没起来」这一解释，门控确实生效。
声学上的自触发另需扬声器→麦克风通路，那一半取决于环境。~~

**以上读法对当前实现已作废。** 它描述的是一次**真实**的观测（当时 `WakeWordEngine` 尚在，
非零的「播报前 12/22」即可为证），但被测的引擎与门控机制**已随唤醒链路重构整体删除并替换**，
所以这条结论无法迁移到当前代码。该组样本从「有效证据」变成了「过时证据」——
后来引擎被删、探针目标消失，同一份检查才退化成只能报空洞 PASS 的假保障（见上方撤回说明）。
**不得再据此宣称当前实现的第 4 项已验证。**

### ❌ 仍未验证（需要人开口说话）

以下 3 项仍需真人对着麦克风说话，本环境无法完成。

| # | 项 | 状态 |
|---|---|---|
| 1 | 播报结束后直接开口说答案 → 被识别并作答 | **未验证**（用验收台第 1 项） |
| 2 | 播报后沉默 8s → 进入待机（界面显示待机文案） | **未验证**（用验收台第 2 项，需安静环境） |
| 3 | 待机态说「小逻小逻」→ 回到本题续答而非开新一轮 | **未验证**（用验收台第 3 项） |
| 5 | 界面按钮作答行为不变 | 由 P4 的既有自动化用例覆盖（非人工项） |

> 验收台自身的判定逻辑做过临时校验（脚本未入库）：窗口开在作答**之前**能命中
> `/api/voice/answer`（1.5s，body 含 session_id 与答案原文），开在作答**之后**采不到；
> 静默窗口新 utter 计数为 0，人为造一条新 utter 后计数为 1 —— 第 3 项赖以区分
> 「续答本题」与「开新一轮」的匹配器确实有效。

**因此 P6 的结论是「自动化全绿 + 运行时冒烟通过；**4 项验收（含原记为已验证的第 4 项）
均待在真人有声环境下用验收台跑一遍**」。** 不得据此宣称语音作答全链路可用。

> **修订（2026-09-13，唤醒链路重构 Task 8）**：本行原为「…+ 第 4 项经验收台验证；第 1/2/3 项
> 仍待…」。第 4 项的那条验证已**对当前实现作废**（理由见上方撤回说明：探针目标随 Vosk 删除、
> 被测门控机制已整体替换），故本行改为 4 项一律待验收。**其余结论（自动化全绿、运行时冒烟）
> 不变**，历史记录保留在上方。
