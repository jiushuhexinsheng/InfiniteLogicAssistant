...
| 执行任意 shell/py | **默认免确认**（2026-09-13 起 `permissions` 默认三档全放行；改回 `ask` 即恢复 confirm） |
| 网络外发（上传/发消息） | 同上 —— 由 `permissions` 策略决定，默认放行 |
| 无人值守（定时任务） | 无确认通道时高风险调用**一律拒绝**（与策略是否放行无关） |

> 当前**未实现**信任记忆 / 黑名单白名单（无"信任该路径"持久化）。
> 确认闸门现由 `permissions` 策略统一控制，**默认关闭**（全放行）；收紧手段是 `permissions.rules`
> 里的 deny/ask 规则或改 `tiers`，见 README「安全」节。
