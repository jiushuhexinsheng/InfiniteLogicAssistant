...
多智能体协作 + 工具执行 → 记忆/RAG 上下文。浏览器端本地 VAD 分段 + 云端唤醒判定 + 轻量 Python
后端（FastAPI），OpenAI 兼容接口，支持离线/在线任意部署。
...
| 语音唤醒 | **VAD → 唤醒判定 → ASR** 三段链路：浏览器本地 VAD 把人声切成段上传，后端转写并判定唤醒词**「衍衡」或「洛吉斯」**（可配多个，含同音字容错）；无本地模型。**隐私边界见「安全」节** |
...
| Skills 技能包 | skills/*.yaml 热加载，{{param}} 填参逐步骤执行；`dangerous: true` 的技能走执行确认 —— **默认已全放行，故默认也不再询问**（见「安全」节） |
...
- 两种说法都支持：
  - **一句话说完**：说「衍衡，帮我查天气」—— 一段里同时有唤醒词和指令，**一次**云端 ASR 就够，
    直接起一轮，没有提示音等待
  - **只喊唤醒词**：说「衍衡」→ 提示音 → 再说指令；唤醒后 8s（`vad.answer_timeout_ms`）内没
    说话则作废，不会把之后无关的一段当成指令
- 唤醒词是 **「衍衡」** 或 **「洛吉斯」**（含同音字容错，如「演横」「罗吉思」）
...
- **播报期间暂停监听**：助手说话时会停掉分段录音器并**释放麦克风轨道**，播完再按上下文恢复，
  避免它自己的声音被采到、切段、当成用户说话上传
- 说出「停止 / 取消 / 暂停」等命令词可中断当前任务
- **对话 / 任务模式**：控制台头部 `[对话][任务]` 开关切换（localStorage 持久化）。
  **对话**（默认）少打断、完成不询问；**任务**模式会在任务完成后询问「这个任务完成了吗？」，答「完成了」才把该任务存档进任务库，下次相似任务自动预填参数减少询问

唤醒词、静音阈值等可在 `config.yaml` 的 `voice.wake_word` / `voice.vad` 中调整。

> **桌面端说明**：桌面原生悬浮球（PySide6）与本地常驻语音监听已暂停开发，桌面代码迁移至 `desktop-ball` 分支（不再回迁）。
> 当前语音交互由浏览器端**本地 VAD 分段 + 云端唤醒判定**与后端 ASR/TTS 承担。

## 架构总览

```
┌──────────────────────────────────────────────────────────────┐
│  交互层  浏览器悬浮球(Vue3 + 本地 VAD + 云端唤醒判定) · ASR/TTS │
├──────────────────────────────────────────────────────────────┤
...
4. **确认** `confirm_if_needed`：由 `permissions` 策略按任务风险层级决定（**默认全放行**，放行时仍播报计划）；无人值守无确认通道时仍一律拒绝
...
> 检查是假保障，比没有检查更糟 —— 这是 2026-09-13 唤醒链路重构专门修掉的一类缺陷。
...
| 4 | 播报含唤醒词 → 不自触发 | 播报期麦克风**活轨道数归零**（门控生效，直接测轨道，不依赖声学通路）**且**播报窗口内无任何音频上传（`/api/voice/wake` / `transcribe` / `answer`） |

第 4 项的判据分两层「原因 + 结果」：麦克风是否真的被释放是**原因**，有没有上传是**结果**。
两者任一为 `FAIL` 即判 `FAIL`；而**任一层拿不到证据**（助手没念出唤醒词、采样没覆盖到播报窗口、
麦克风探针不可用、或播报前麦克风本来就是关的 —— 没有东西可挡）一律判 `INCONCLUSIVE`，不判 `PASS`。
...
> 与「待答超时没生效」的缺陷区分开。
> **第 4 项的声学部分**（播报窗口内无音频上传）取决于扬声器→麦克风的实际通路；扬声器静音时
> 它无判别力，但「播报期麦克风活轨道数归零」这一半直接测轨道，不依赖声学通路，任何情况下
> 都可证伪。
> 脚本会在报告里附上**探针是否可用**与**播报前麦克风是否真的开着**，所以「全都 0」不会被
> 误读成通过。

### 唤醒链路重构（P8 子项目 1）人工验收状态

⛔ **以下 6 项必须由真人对着麦克风在真实有声环境下逐条执行，实现者不得代劳、不得代填。**
自动化只能覆盖其中客观可判的部分（见上表），而 spec 里的音频实测用的是 **SAPI 合成音** ——
云端对合成音识别好**不代表对真人好**。**在全部通过之前，不得宣称唤醒功能可用。**

| # | 验收项 | 结果 |
|---|--------|------|
| 1 | 说「衍衡，帮我查天气」→ 一句话走通（**一次** ASR，无提示音等待） | **待人工验收** |
| 2 | 说「衍衡」→ 提示音 → 说指令 → 走通 | **待人工验收** |
| 3 | 说「洛吉斯，现在几点」→ 走通 | **待人工验收** |
| 4 | 说「衍衡」后不说话 → 不产生任务，不报错 | **待人工验收** |
| 5 | 屋里正常聊天（不含唤醒词）→ **不误触发** | **待人工验收** |
...
- **密钥零落库**：`config.yaml` 不含密钥，密钥在 `config.secrets.yaml` / 环境变量；API 永不回显密钥值，
  设置页只报「已设置 / 未设置」。
- **无沙箱 + 人类在环（策略可配，**默认已全放行**）**：工具是否需要操作者确认由 `permissions` 策略决定
  （设置页「权限」可改）。**2026-09-13 起默认三档全放行** —— 即默认**不询问**：`run_shell_tool` 可执行
  任意命令、`write_file` 可覆盖任意文件，都不会先问一句。收紧方式：把某档改回 `ask`，或加 `deny` 规则
  （按工具名 glob 匹配，deny 单调短路、不可被后续规则翻案）。
  策略求值顺序：未注册工具 → 拒绝；任一 `deny` 规则短路；首条匹配规则；层级默认；`default_action`。
  任务开始时的**计划级确认**与逐工具确认**共用同一份 `permissions`**（按任务的 `risk` 层级判定）；
  放行时**仍会播报计划**，只是不阻塞等待 —— 你依然看得到助手要做什么，只是不用再点一次。
  无人值守（定时任务）不受影响：无确认通道时高风险调用一律拒绝。
  **全放行等于这道闸门默认不生效**，请确认你接受该风险。
- **语音隐私边界（2026-09-13 唤醒链路重构后）**：语音唤醒改走**云端判定**，因此
  **每次本地 VAD 判到人声，都会把该音频片段上传到云端 ASR（你 `config.yaml` 里 `asr` 指向的
  endpoint，`config.yaml.example` 默认留空）—— 无论是否
...
（见「安全」节：2026-09-13 起默认全放行），需要询问时在 `permissions` 里把对应层级改回 `ask`。
...
│       │   └── assistant/      store / useChat / useTts / useWakeWord / useSegmentRecorder（本地 VAD 分段）/ wakeFsm
