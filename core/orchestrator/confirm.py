...
async def confirm_if_needed(task: Task, plan: str, session: Session) -> bool:
    ...
    if session.channel is None:
        audit(f"confirm risk={task.risk} plan={plan} decision=rejected reason=no_operator")
    ...
    if any(w in ans for w in _CONFIRM_YES):
        audit(f"confirm risk={task.risk} plan={plan} decision=approved answer={ans!r}")
        return True
    if any(w in ans for w in _CONFIRM_NO):
        audit(f"confirm risk={task.risk} plan={plan} decision=rejected answer={ans!r}")
        return False
    audit(f"confirm risk={task.risk} plan={plan} decision=rejected answer={ans!r} reason=ambiguous")
